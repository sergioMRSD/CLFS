...
def main():
    ...
    summary = {
        ...
        "CIK_001": 0,
        "OTH_001": 0,
        # survey_dataset_v2 specific rules
        "GMI_001": 0,
        "HMGC_001": 0,
    }
    ...
    for idx, row in df.reset_index(drop=True).iterrows():
        ...
        # --- survey_dataset_v2 checks (GMI and household marital/gender consistency)
        # GMI: flag if > 50000
        if "GMI" in cols:
            gmi_v = safe_number(r.get("GMI"))
            if gmi_v is None:
                skipped["GMI_001"] += 1
            else:
                if gmi_v > 50000:
                    triggered.append("GMI_001")
                    if "GMI" not in t_cells:
                        t_cells.append("GMI")
                    summary["GMI_001"] += 1

        # Household marital/gender consistency for two individuals: sex_1/marital_status_1 and sex_2/marital_status_2
        # Apply flags for mismatches or missing partner data as specified.
        ...
        for c in cols:
            cl = c.lower()
            if cl in {"marital_status_1", "marital status 1", "marital status_1"}:
                ms1_col = c
            if cl in {"marital_status_2", "marital status 2", "marital status_2"}:
                ms2_col = c
            if cl in {"sex_1", "sex 1", "sex1"}:
                sex1_col = c
            if cl in {"sex_2", "sex 2", "sex2"}:
                sex2_col = c
        # If not found, fallback to exact names if present
        if not ms1_col and "marital_status_1" in cols:
            ms1_col = "marital_status_1"
        if not ms2_col and "marital_status_2" in cols:
            ms2_col = "marital_status_2"
        if not sex1_col and "sex_1" in cols:
            sex1_col = "sex_1"
        if not sex2_col and "sex_2" in cols:
            sex2_col = "sex_2"

        # Only evaluate if at least one of the pair columns exists
        if ms1_col or ms2_col or sex1_col or sex2_col:
            m1 = safe_str(r.get(ms1_col)) if ms1_col else ""
            m2 = safe_str(r.get(ms2_col)) if ms2_col else ""
            s1 = safe_str(r.get(sex1_col)) if sex1_col else ""
            s2 = safe_str(r.get(sex2_col)) if sex2_col else ""
            ...
            # consider values 'married' to match
            is_m1 = "married" in m1_l
            is_m2 = "married" in m2_l

            flagged_hmgc = False
            # case a) both married but same gender
            if is_m1 and is_m2 and s1 and s2:
                if s1_l == s2_l:
                    flagged_hmgc = True
            # case b) one married but other not married
            if (is_m1 and not is_m2) or (is_m2 and not is_m1):
                flagged_hmgc = True
            # case c) married exists but partner's data missing or inconsistent
            if (is_m1 and (not m2 and not s2)) or (is_m2 and (not m1 and not s1)):
                flagged_hmgc = True

            if flagged_hmgc:
                triggered.append("HMGC_001")
                # highlight the relevant fields if present
                for colname in (ms1_col, sex1_col, ms2_col, sex2_col):
                    if colname and colname not in t_cells:
                        t_cells.append(colname)
                summary["HMGC_001"] += 1
        else:
            # nothing to evaluate
            skipped["HMGC_001"] += 1
