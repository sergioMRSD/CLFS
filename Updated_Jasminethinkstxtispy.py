...
COUNTRY_LIST = {
    ...
    "spain", "sri lanka", "st. vincent & grenadines", "state of palestine",
...
def _member_suffix(member_idx: int) -> str:
    s = member_idx - 1
    return "" if s == 0 else f".{s}"


def _find_member_column_name(columns: list, target: str, member_idx: int) -> Optional[str]:
    """Find the best column name for a member-specific question.

    Prefer exact suffixed columns (e.g., 'Employment Status as of last week.1') for member_idx>1.
    Fall back to the base column if no suffixed column exists.
    """
    if not columns:
        return None
    cols = list(columns)
    target_norm = _normalize_header(target)
    suffix = _member_suffix(member_idx)

    # Try exact suffixed header match first
    if suffix:
        for c in cols:
            if _normalize_header(c) == target_norm + suffix:
                return c
        # Then try partial matches that include the target and end with suffix
        for c in cols:
            h = _normalize_header(c)
            if h.endswith(suffix) and target_norm in h:
                return c

    # Fallback: return generic find
    return _find_column_name(cols, target)


def _get_member_cell_value(df_obj: pd.DataFrame, row_idx: int, target: str, member_idx: int) -> Optional[object]:
    col = _find_member_column_name(list(df_obj.columns), target, member_idx)
    if not col:
        return None
    return df_obj.at[row_idx, col]


def _get_member_column_index(df_obj: pd.DataFrame, target: str, member_idx: int) -> tuple[Optional[str], Optional[int]]:
    col = _find_member_column_name(list(df_obj.columns), target, member_idx)
    if not col:
        return None, None
    return col, df_obj.columns.get_loc(col)
...
def extract_household_members(df: pd.DataFrame) -> list[list[HouseholdMember]]:
    """
    Extract household members using fixed-width blocks per user request.

    Rules implemented:
    - Read member count from Column E (index 4).
    - Each member occupies exactly 209 columns.
    - First member block starts at Column G (index 6).
    - For N members, slice N blocks and only consider those columns when
      extracting member attributes to avoid ghost errors from trailing columns.
    """
    ...
    for row_idx, row in df.iterrows():
        ...
        # Read member count from Column E (index 4). Fall back to 0 on error.
        try:
            raw_count = row.iloc[4]
            member_count = int(float(raw_count)) if raw_count not in (None, "", "nan") else 0
        except Exception:
            member_count = 0

        for m in range(member_count):
            start_idx = 6 + m * 209
            end_idx = start_idx + 209
            if start_idx >= total_cols:
                # No more columns available for this member; stop early
                break
            block_cols = columns[start_idx:end_idx]

            # Find full name within the block to decide whether member exists
            fn_col = _find_column_name(block_cols, "Full Name")
            name = None
            if fn_col:
                # global column name -> get value
                name = _normalize_value(row.at[fn_col])
            # If no name found, still create member if any block has data (to preserve indexing)
            if not name:
                # quick check: any non-empty cell in block
                any_val = False
                for c in block_cols:
                    try:
                        v = _normalize_value(row.at[c])
                        if v:
                            any_val = True
                            break
                    except Exception:
                        continue
                if not any_val:
                    # empty block, skip member
                    continue
                name = ""

            member = HouseholdMember(full_name=name)
            # attach block columns to member to restrict lookups later
            setattr(member, "_block_columns", block_cols)

            # Populate attributes by searching only within block columns
            for attr_name, col_name in COLUMN_MAPPING.items():
                if attr_name == "full_name":
                    continue
                matched_col = _find_column_name(block_cols, col_name)
                if matched_col:
                    try:
                        value = _normalize_value(row.at[matched_col])
                    except Exception:
                        value = None
                    if value:
...
# Prefer an explicitly provided path via env var, otherwise prefer a project-root
# upload of the ssoc definitions, and finally fall back to the references/ path.
_project_root_defs = Path(__file__).resolve().parent / "ssoc2024-detailed-definitions.xlsx"
# pyright: reportMissingImports=false, reportGeneralTypeIssues=false, reportOptionalMemberAccess=false
# type: ignore

SSOC_DEFINITIONS_FILE = os.environ.get(
    "SSOC_DEFINITIONS_FILE",
    str(_project_root_defs if _project_root_defs.exists() else (Path("references") / "ssoc2024-detailed-definitions.xlsx"))
)
# How many initial metadata rows to skip in the SSOC definitions file before detecting header.
# Use an env var to override if needed. Default to 4 (typical for exported files with 4-5 metadata rows).
SSOC_DEF_SKIP_ROWS = int(os.environ.get("SSOC_DEF_SKIP_ROWS", "4"))
...
def _load_ssoc_resources() -> Optional[dict]:
    ...
    # Call the SSOC loader with an explicit skip-rows value so files that have
    # 4-5 metadata rows before the true header are parsed correctly.
...
# ==============================================================================
# AMENDED: VALIDATION REPORT LOGIC (DYNAMIC HORIZONTAL FORMAT)
# ==============================================================================
def create_validation_report(
    ...
    """
    Creates a validation report in the requested dynamic horizontal format.
    Format: file, row, response_id, member, rule, column, message, corrections, 
    (additional column, message, corrections for multi-variable rules), Remarks
    """
    if not rule_errors:
        return None
    ...
    # Pre-process errors to determine maximum number of dependent variables
    for err in rule_errors:
        entry = {
            "file": err.get("file"),
            "row": err.get("row"),
            "response_id": err.get("response_id"),
            "member": err.get("member"),
            "rule": err.get("rule"),
        }
        
        # Split headers if '&' exists (multi-variable rules)
        column_input = str(err.get("column", ""))
        column_list = [c.strip() for c in column_input.split("&")]
        message_text = err.get("message", "")
        
        if len(column_list) > max_vars:
            max_vars = len(column_list)

        # Distribute into numbered dynamic columns
        for i, col_name in enumerate(column_list):
            suffix = "" if i == 0 else f"_{i+1}"
            entry[f"column{suffix}"] = col_name
            entry[f"message{suffix}"] = message_text # Consistent message across row
            entry[f"corrections{suffix}"] = ""

        # Store internal reference to row for Remarks mapping
        entry["_orig_row"] = err.get("row")
        processed_details.append(entry)

    details_df = pd.DataFrame(processed_details)

    # 1. Build Header Sequence
    base_headers = ["file", "row", "response_id", "member", "rule"]
    dynamic_headers = []
    for i in range(max_vars):
        suffix = "" if i == 0 else f"_{i+1}"
        dynamic_headers.extend([f"column{suffix}", f"message{suffix}", f"corrections{suffix}"])

    # 2. Extract Remarks context from original validated data
    remarks_values = []
    if validated_df is not None and not validated_df.empty:
        remarks_col = _find_column_name(list(validated_df.columns), "Remarks")
        if remarks_col:
            max_df_idx = len(validated_df) - 1
            for row_val in details_df["_orig_row"]:
                try:
                    idx = int(row_val) - 1
                    val = validated_df.at[idx, remarks_col] if 0 <= idx <= max_df_idx else ""
                    remarks_values.append("" if pd.isna(val) else str(val))
                except:
                    remarks_values.append("")
        else:
            remarks_values = [""] * len(details_df)
    else:
        remarks_values = [""] * len(details_df)

    details_df["Remarks"] = remarks_values

    # 3. Finalize column order and filter non-existent numbered columns
    final_order = [c for c in base_headers + dynamic_headers + ["Remarks"] if c in details_df.columns]
    details_df = details_df[final_order]

    # 4. Create Summary using base column/message
    summary_df = (
        details_df.groupby(["rule", "column", "message"], dropna=False)
        .size()
        .reset_index(name="count")
        .sort_values("count", ascending=False)
    )

    # 5. Save to Excel and apply specific formatting
    ...
    # Highlight corrections columns in Details sheet
    if "Details" in wb.sheetnames:
        ws_details = wb["Details"]
        for col_idx, col_name in enumerate(details_df.columns, 1):
            if "corrections" in str(col_name):
                for row in range(1, ws_details.max_row + 1):
                    ws_details.cell(row=row, column=col_idx).fill = yellow_fill

    # Copy visual highlights to complete dataset sheet
    if "Complete Dataset" in wb.sheetnames and (changes or error_cells):
        _apply_dataset_highlights(wb["Complete Dataset"], changes or {}, error_cells or set())
    ...
    for filename, df in files.items():
        ...
        if not ssoc_groups:
            ...
            for row_idx in range(len(df)):
                for group_idx, group in enumerate(ssoc_groups):
                    ...
                    # Use column name to assign to avoid dtype/positional issues
                    ssoc_col_name = modified_df.columns[ssoc_idx]
                    old_val = modified_df.iat[row_idx, ssoc_idx]
                    if str(old_val).strip() != str(ssoc_code).strip():
                        try:
                            # Ensure the SSOC column is object dtype to allow string codes
                            if str(modified_df.dtypes[ssoc_col_name]) != 'object':
                                modified_df[ssoc_col_name] = modified_df[ssoc_col_name].astype(object)
                            modified_df.at[row_idx, ssoc_col_name] = ssoc_code
        ...
        if ssic_col and STRATA_LOOKUP:
            ...
            if est_col and ssic_matched_col is not None and ssic_idx is not None:
                for row_idx in range(len(df)):
                    ...
                    if match:
                        ssic_col_name = modified_df.columns[ssic_idx]
                        old_val = modified_df.iat[row_idx, ssic_idx]
                        if str(old_val).strip() != str(match).strip():
                            try:
                                # Ensure the SSIC column is object dtype to allow string codes
                                if str(modified_df.dtypes[ssic_col_name]) != 'object':
                                    modified_df[ssic_col_name] = modified_df[ssic_col_name].astype(object)
                                modified_df.at[row_idx, ssic_col_name] = match
        ...
        for household_idx, members in enumerate(households, 1):
            for member_idx, member in enumerate(members, 1):
                ...
                # Member-aware column helpers: wide format uses suffixes (.1, .2, ...) for member 2,3,...
                def _member_suffix(idx: int) -> str:
                    # member_idx is 1-based; member 1 has no suffix, member 2 -> .1
                    s = idx - 1
                    return "" if s == 0 else f".{s}"

                def _find_member_column_name(columns: list, target: str, m_idx: int) -> Optional[str]:
                        # Prefer searching inside the member's fixed block columns if available
                        cols = list(columns)
                        target_norm = _normalize_header(target)
                        # If the member has a restricted block, search only inside it first
                        block_cols = getattr(member, "_block_columns", None)
                        if block_cols:
                            # exact match inside block
                            for c in block_cols:
                                if _normalize_header(c) == target_norm:
                                    return c
                            # partial matches inside block
                            partials = [c for c in block_cols if target_norm in _normalize_header(c)]
                            if partials:
                                return sorted(partials, key=lambda c: len(_normalize_header(c)))[0]

                        # Prefer exact suffixed column for the member in the full columns list; fall back to base if not found
                        suffix = _member_suffix(m_idx)
                        if suffix:
                            for c in cols:
                                if _normalize_header(c) == target_norm + suffix:
                                    return c
                        ...
                        # Fallback to generic finder which prefers exact/shortest match
                        return _find_column_name(cols, target)

                def _get_member_cell_value(df_obj: pd.DataFrame, r_idx: int, target: str, m_idx: int) -> Optional[object]:
                    col = _find_member_column_name(list(df_obj.columns), target, m_idx)
                    if not col:
                        return None
                    return df_obj.at[r_idx, col]

                def _get_member_column_index(df_obj: pd.DataFrame, target: str, m_idx: int) -> tuple[Optional[str], Optional[int]]:
                    col = _find_member_column_name(list(df_obj.columns), target, m_idx)
                    if not col:
                        return None, None
                    return col, df_obj.columns.get_loc(col)
                ...
                if member.age_started_employment is not None:
                    ...
                    if not result.is_valid:
                        ...
                        matched_col, col_idx = _get_member_column_index(df, col_name, member_idx)
                ...
                if member.usual_hours_of_work is not None:
                    ...
                    if not result.is_valid:
                        matched_col, col_idx = _get_member_column_index(df, "Usual hours of work", member_idx)
                        if matched_col is not None and col_idx is not None:
                            error_cells.add((row_idx, col_idx))
                            rule_errors.append({
                                "file": filename,
                ...
                if member.freelance_online_platforms_last_12_months is not None:
                    ...
                    if not result.is_valid:
                        # Highlight both employment status and freelance columns (member-specific)
                        ...
                        emp_matched, emp_idx = _get_member_column_index(df, emp_col, member_idx)
                        if emp_matched is not None and emp_idx is not None:
                            error_cells.add((row_idx, emp_idx))
                        free_matched, free_idx = _get_member_column_index(df, free_col, member_idx)
                ...
                # Skip RULE 19 if employment status explicitly indicates Employee for this member
                emp_status_norm = _normalize_text(member.employment_status_last_week)
                if emp_status_norm and "employee" in emp_status_norm:
                    # Member is an employee; freelance consistency rule does not apply
                    pass
                else:
                    if freelance_val and freelance_val != _normalize_text(NO_FREELANCE_TEXT):
                        ...
                        if se_val != "yes" or oa_val != "yes":
                            ...
                            free_col = "Did you perform any freelance or assignment-based work via any of the following online platform(s) in the last 12 months?"
                            self_matched, self_idx = _get_member_column_index(df, self_col, member_idx)
                            own_matched, own_idx = _get_member_column_index(df, own_col, member_idx)
                            free_matched, free_idx = _get_member_column_index(df, free_col, member_idx)
        ...
        # Prefix member info to all member-level error messages so logs are explicit
        for err in rule_errors:
            mi = err.get("member_index")
            mem = err.get("member")
            if mi is not None and mem:
                msg = err.get("message") or ""
                prefix = f"Member {mi} [{mem}]: "
                if not str(msg).startswith(prefix):
                    err["message"] = prefix + str(msg)

        # Create validation report (summary + details + complete dataset)
        report_path = create_validation_report(rule_errors, filename, modified_df, changes, error_cells)

        # Save validated output if changes were made
        validated_path = None
        if changes or error_cells:
            original_path = Path("Operating_Table") / filename
            validated_path = save_with_highlights(modified_df, str(original_path), changes, error_cells)

        # Also copy outputs to the requested standardized filenames in /output
        output_dir = create_output_directory()
        try:
            if validated_path and Path(validated_path).exists():
                dest_validated = output_dir / "CLFS_newformat_validated.xlsx"
                shutil.copy(Path(validated_path), dest_validated)
                print(f"Copied validated file to: {dest_validated}")
            if report_path and Path(report_path).exists():
                dest_report = output_dir / "CLFS_newformat_validation_report.xlsx"
                shutil.copy(Path(report_path), dest_report)
                print(f"Copied validation report to: {dest_report}")
        except Exception as e:
            print(f"Warning: failed to copy outputs to standardized names: {e}")
